#!/bin/bash
#
# mysql-fractalsql multi-arch Docker-driven build.
#
...
#
# One .so per arch covers MySQL 8.0 / 8.4 LTS / 9.x — the UDF ABI has
# been stable since MySQL 8.0.0, so per-major fan-in would produce
# identical binaries.
#
# Usage:
...
echo "------------------------------------------"
echo "Building mysql-fractalsql for ${PLATFORM}"
