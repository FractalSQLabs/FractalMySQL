/* src/fractalsql.c
 * mysql-fractalsql v1.0 — Stochastic Fractal Search for MySQL (UDF).
 *
 * Compatible with MySQL 8.0, 8.4 LTS, and 9.x (rolling). The UDF ABI
 * has been stable since MySQL 8.0.0 — UDF_INIT / UDF_ARGS, the
 * init/main/deinit signatures, and MYSQL_ERRMSG_SIZE (512) have not
 * changed. One fractalsql.so per arch covers every supported major;
 * release packages depend on mysql-server generically.
 *
 * MySQL 8.0+ uses C `bool` (<stdbool.h>) as the UDF init return type.
 * The legacy `my_bool` typedef was removed in 8.0, so this TU includes
 * <stdbool.h> and returns `bool` directly. Building this source against
 * a pre-8.0 libmysqlclient header will fail; 5.7 is out of scope.
 *
 * MySQL 9.0 introduced a native VECTOR type with its own binary
 * encoding (distinct from MariaDB 11.7+'s VECTOR). This release
 * accepts only CSV / bracketed-JSON inputs; a follow-up iteration can
 * enable a -DFRACTAL_HAVE_MYSQL9_VECTOR_TYPE decode path behind the
 * #ifdef block below.
 *
 * SQL surface
 *   fractal_search(vector_csv, query_csv, k, params) -> JSON STRING
 *   fractalsql_edition() -> STRING  ('Community')
 *   fractalsql_version() -> STRING  ('1.0.0')
 *
 ...
 *     stable thread affinity across calls, so a fresh state is built in
 *     *_init, the pointer travels through initid->ptr, and it is closed
 *     in *_deinit. No global Lua state, no cross-call locking.
 *   * The LuaJIT optimizer ships as pre-stripped bytecode embedded via
 *     include/sfs_core_bc.h. No Lua source exists at runtime.
 */
...
#ifdef FRACTAL_HAVE_MYSQL9_VECTOR_TYPE
 ...
 *
 * Deferred to a follow-up iteration — the v1.0.0 release accepts only
 * CSV / bracketed-JSON inputs, which covers every current MySQL major
 * identically. */
static bool
...
#endif  /* FRACTAL_HAVE_MYSQL9_VECTOR_TYPE */
