...
# mysql-fractalsql by FractalSQLabs

**Stochastic Fractal Search as a MySQL UDF.** One function, four
arguments, a JSON document back.

mysql-fractalsql ships a LuaJIT-backed metaheuristic optimizer that
runs inside the MySQL server process. You hand it a corpus of vectors,
a query, how many matches you want, and a tuning blob — it returns the
continuous best point plus the top-k stored vectors as a single JSON
string that MySQL's own `JSON_EXTRACT()` can slice.
...
- **One .so per Linux arch** — the MySQL UDF ABI (`UDF_INIT`,
  `UDF_ARGS`, `MYSQL_ERRMSG_SIZE`, the init/main/deinit signatures)
  has been stable since MySQL 8.0.0, so the same binary works on
  every supported major. The `.deb` / `.rpm` depends on
  `mysql-server` generically.
...
**Linux.** One `fractalsql.so` per arch covers every listed MySQL
major — no per-major fan-in. CI smoke-tests the same binary against
both `mysql:8.0` and `mysql:8.4` containers on every release tag,
which is the actual cross-major ABI proof.
**Windows.** Oracle does not publish 32-bit or ARM64 Windows server
binaries on the current MySQL majors (8.0+), so both cells are
deferred until upstream host binaries exist. Windows x64 ships one
MSI per MySQL major because the install path is major-specific
(`C:\Program Files\MySQL\MySQL Server <VER>\lib\plugin\`).

**MySQL 5.7.** Out of scope. MySQL 8.0 changed `my_bool` to plain
`bool` in its public UDF headers; this source targets the 8.0+ ABI
and will not compile against 5.7.

**MySQL 9.0 VECTOR type.** MySQL 9.0 introduced a native VECTOR type
with its own binary encoding (distinct from MariaDB 11.7+'s VECTOR).
The v1.0.0 UDF signature accepts only CSV / bracketed-JSON inputs —
VECTOR columns work transparently via an explicit `CAST`/text
conversion. A dedicated `-DFRACTAL_HAVE_MYSQL9_VECTOR_TYPE` decode
path is flagged for a follow-up iteration.
...
Slice it with MySQL's JSON functions — no client-side parsing
needed:
...
> **CONVERT USING utf8mb4.** MySQL 8.0+ tags UDF `STRING` returns with
> `CHARACTER SET 'binary'`, and `JSON_EXTRACT` rejects binary-charset
> inputs with `ERROR 3144 (22032)`. Re-tagging the payload as
> `utf8mb4` is a no-op on the underlying bytes (it's pure ASCII JSON)
> and unblocks every JSON function. Put the `CONVERT(... USING
> utf8mb4)` anywhere the result is about to meet `JSON_EXTRACT`,
> `JSON_VALUE`, `->`, `->>`, etc. MariaDB's JSON functions are
> permissive and don't need the wrap.
...
[GitHub Releases](https://github.com/FractalSQLabs/mysql-fractalsql/releases).
One binary covers MySQL 8.0 / 8.4 LTS / 9.x:

```bash
# amd64 / x86_64
sudo apt install ./mysql-fractalsql-amd64.deb
# or
sudo rpm -i mysql-fractalsql-amd64.rpm

# arm64 / aarch64 (AWS Graviton, Apple Silicon, Ampere Altra, …)
sudo apt install ./mysql-fractalsql-arm64.deb
sudo rpm -i mysql-fractalsql-arm64.rpm
```

The package drops `fractalsql.so` into `/usr/lib/mysql/plugin/` and
the registration script + LICENSE files into
`/usr/share/mysql-fractalsql/` / `/usr/share/doc/mysql-fractalsql/`.
Activate once:

```sql
SOURCE /usr/share/mysql-fractalsql/install_udf.sql;
-- or from the shell:
-- mysql -u root -p < /usr/share/mysql-fractalsql/install_udf.sql
```
...
Download the MSI matching your MySQL major from
[GitHub Releases](https://github.com/FractalSQLabs/mysql-fractalsql/releases):

```
FractalSQL-MySQL-8.0-1.0.0-x64.msi
FractalSQL-MySQL-8.4-1.0.0-x64.msi
FractalSQL-MySQL-9.1-1.0.0-x64.msi
```

Interactive install (standard Windows double-click flow):

```powershell
msiexec /i FractalSQL-MySQL-8.4-1.0.0-x64.msi
```

Silent install (scripting / MDM):

```powershell
msiexec /i FractalSQL-MySQL-8.4-1.0.0-x64.msi /qn
```

Targeting a non-default MySQL install root (e.g.
`D:\MySQL\MySQL Server 8.4`):

```powershell
msiexec /i FractalSQL-MySQL-8.4-1.0.0-x64.msi /qn ^
    MYSQLROOT="D:\MySQL\MySQL Server 8.4"
```

The MSI drops `fractalsql.dll` into
`C:\Program Files\MySQL\MySQL Server <VER>\lib\plugin\` and the
install SQL plus LICENSE files into
`C:\Program Files\MySQL\MySQL Server <VER>\share\doc\mysql-fractalsql\`.
Activate once from the MySQL client:

```powershell
mysql -u root -p < "C:\Program Files\MySQL\MySQL Server 8.4\share\doc\mysql-fractalsql\install_udf.sql"
```
...
`default-libmysqlclient-dev`, and runs `docker/assert_so.sh` to
verify the zero-dependency posture (ldd shortlist, no
`__cxx11::basic_string` leaks, size ceiling, UDF entry points in
`.dynsym`). Cross-arch builds use buildx + QEMU; CI runs both arches
on every tag.

For quick local iteration against whatever libmysqlclient-dev is on
your path (does NOT produce a shipping artifact — uses dynamic
LuaJIT linkage):

```bash
sudo apt install -y build-essential default-libmysqlclient-dev libluajit-5.1-dev pkg-config
make
sudo make install
...
Requires a Developer Command Prompt for Visual Studio, a MySQL
Windows binaries `.zip` unpacked (for `include\mysql.h`), and the
WiX Toolset if you want to rebuild the MSI:
...
set MYSQL_DIR=C:\path\to\unpacked\mysql-8.4.4-winx64
set MYSQL_MAJOR=8.4
...
mysql-fractalsql skips all of this: the bytecode is compiled once at
...
MYSQL_IMAGE=mysql:9.1  \
  docker compose -f benchmark/docker-compose.test.yml up --abort-on-container-exit
```

The compose file pins each container to a single dedicated CPU core
(MySQL on core 0, Node tester on core 1) so latency numbers stay
...
**One Lua state per UDF invocation.** MySQL is multi-threaded with no
stable thread affinity across calls. The plugin constructs a fresh
...
**Determinism.** LuaJIT's `math.random` is xoshiro256\*\*. Because
each call builds a fresh Lua state, results are reproducible across
calls when you pin `population_size` and seed `math.randomseed` in a
custom build.
...
mysql-fractalsql embeds two third-party components. Full notices
live in [LICENSE-THIRD-PARTY](LICENSE-THIRD-PARTY) at the repo root
(and inside every installed package under
`/usr/share/doc/mysql-fractalsql/` on Linux,
`\share\doc\mysql-fractalsql\` on Windows).
